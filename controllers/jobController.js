...
// GET /api/jobs/ticker this is for the alert bar on homepage which shows the latest and most relevant jobs in a ticker format, it uses a smart ranking algorithm to rank the jobs based on their relevance and urgency.
const getTickerJobs = async (req, res) => {
  try {
    const jobs = await Job.find({})
      .sort({ createdAt: -1 })
      .limit(8)
      .select("companyName jobTitle slug");

    const tickerJobs = jobs.map((job) => ({
      title: `${job.companyName} is hiring for ${job.jobTitle}`,
      slug: job.slug,
